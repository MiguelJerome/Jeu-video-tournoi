...
app.post("/acceuil", async (request, response) => {
  if (!request.user) {
    ...
    let id = await addTournoiInscrit(
      request.body.id_tournois,
...
app.delete("/compte", async (request, response) => {
  if (request.user) {
    response.render("compte", {
      ...
      accept: request.session.accept,
      scripts: ["/js/compte.js"],
      tournois: await deleteTournoiInscrit(
        request.body.id_tournois,
        request.user.id_utilisateur
      ),
      adminLogin: request.user?.id_type_utilisateur != 2,
...
app.get("/accueil/id", async (req, res) => {
  /**
   * ! Quand je protege cette route avec (req.user) ca marche pas, je te laisse demander au prof demain.
   */
  if (req.user.id_type_utilisateur !== undefined) {
